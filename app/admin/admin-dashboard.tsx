...
export function AdminDashboard({ userName }: { userName: string }) {
  ...
  const stats = useMemo(() => ({ total: products.length, active: products.filter((item) => item.active).length, available: products.filter((item) => item.stock > 0).length }), [products]);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const payload = { ...form, priceCents, compareAtPriceCents, stock: form.stock > 0 ? 1 : 0, imageUrl, imageKey };
  ...
  async function toggleAvailability(product: Product) {
    ...
    setMutatingProductId(product.id); setNotice(null);
    try {
      const response = await fetch(`/api/products/${product.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ stock: nextStock }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      const confirmed = await load();
      const savedProduct = confirmed?.find((item) => item.id === product.id);
      if (!savedProduct || (savedProduct.stock > 0) !== (nextStock > 0)) throw new Error("A disponibilidade não pôde ser confirmada. Recarregue o painel.");
      setNotice({ type: "success", text: `${product.name} agora está ${nextStock > 0 ? "disponível" : "esgotado"} na loja.` });
    } catch (error) {
      setNotice({ type: "error", text: error instanceof Error ? error.message : "Não foi possível alterar a disponibilidade." });
    } finally { setMutatingProductId(null); }
  }
  ...
  return (
    <main className="admin-shell">
      ...
      {view === "catalog" ? <section className="admin-main" id="catalogo">
        ...
        <div className="admin-stats"><article><span>Produtos cadastrados</span><strong>{String(stats.total).padStart(2, "0")}</strong></article><article><span>Visíveis na loja</span><strong>{String(stats.active).padStart(2, "0")}</strong></article><article><span>Disponíveis para compra</span><strong>{String(stats.available).padStart(2, "0")}</strong></article></div>
        <div className="admin-table-wrap"><div className="admin-table-heading"><h2>Seus relógios</h2><span>{products.length} produtos</span></div>
          {loading ? <div className="admin-loading">Carregando catálogo...</div> : products.length ? <div className="admin-products">{products.map((product) => <article className={`admin-product ${product.stock === 0 ? "is-sold-out" : ""}`} key={product.id}>
            <div className="admin-product-image"><img src={product.imageUrl} alt="" loading="lazy" decoding="async" />{product.stock === 0 && <span>Esgotado</span>}</div>
            <div className="admin-product-name"><span>{product.category}</span><strong>{product.name}</strong><small>{product.stock > 0 ? "Disponível para compra" : "Marcado como esgotado"}</small></div>
            <div className="admin-product-price"><span>Preço</span><strong>{money(product.priceCents)}</strong></div>
            <div className="admin-product-controls"><label className="status-toggle"><input type="checkbox" checked={product.active} disabled={Boolean(mutatingProductId)} onChange={() => toggle(product)} /><span />{product.active ? "Visível" : "Oculto"}</label><button type="button" className={product.stock > 0 ? "sold-out-action" : "sold-out-action restore"} disabled={Boolean(mutatingProductId)} onClick={() => toggleAvailability(product)}>{product.stock > 0 ? "Marcar esgotado" : "Disponibilizar"}</button></div>
            <div className="row-actions"><button disabled={Boolean(mutatingProductId)} onClick={() => openEdit(product)}>Editar</button><button className="danger" disabled={Boolean(mutatingProductId)} onClick={() => remove(product)}>Excluir</button></div>
          </article>)}</div> : <div className="admin-empty"><h3>Seu catálogo está vazio.</h3><p>Adicione o primeiro relógio para ele aparecer na loja.</p><button className="button primary" onClick={openNew}>Adicionar produto</button></div>}
        </div>
