...
import { defaultStoreSettings, formatShippingAddress, money, normalizeWhatsapp, orderStatusLabels, renderOrderMessage, type Order, type OrderStatus, type StoreSettings } from "@/lib/commerce";
...
export function CommerceDashboard({ view }: { view: AdminView }) {
  ...
  return <section className="admin-main admin-commerce">
    ...
    {!loading && view === "settings" && <p className="template-address-help">Para incluir o endereço completo nas mensagens, use a variável <code>{`{endereco}`}</code>.</p>}
...
function OrderList({ orders, saving, onStatus, onContact }: { orders: Order[]; saving: boolean; onStatus: (order: Order, status: OrderStatus) => void; onContact: (order: Order) => void }) {
  ...
  return <div className="commerce-orders">{orders.map((order) => <article className="commerce-order" key={order.id}>
    <header><div><span>{sourceLabels[order.source]}</span><strong>{order.code}</strong><small>{formatDate(order.createdAt)}</small></div><select disabled={saving} value={order.status} aria-label={`Status do pedido ${order.code}`} onChange={(event) => onStatus(order, event.target.value as OrderStatus)}>{Object.entries(orderStatusLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></header>
    <div className="order-customer"><strong>{order.customer.name}</strong><a href={`https://wa.me/${normalizeWhatsapp(order.customer.whatsapp)}`} target="_blank" rel="noreferrer">+{normalizeWhatsapp(order.customer.whatsapp)}</a>{order.customer.instagram && <a href={`https://instagram.com/${order.customer.instagram}`} target="_blank" rel="noreferrer">@{order.customer.instagram}</a>}</div>
    {order.customer.address && <address className="order-address"><span>Endereço de entrega</span><strong>{formatShippingAddress(order.customer.address)}</strong></address>}
    <div className="order-items">{order.items.map((item, index) => <p key={`${item.productId}-${index}`}><span>{item.quantity}x {item.simplifiedName}</span><strong>{money(item.unitPriceCents * item.quantity)}</strong></p>)}</div>
    <footer><div><span>Total</span><strong>{money(order.totalCents)}</strong></div><button className="button primary" onClick={() => onContact(order)}>Chamar no WhatsApp <span>↗</span></button></footer>
  </article>)}</div>;
}
