...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const payload = await readJsonBody<{ status?: unknown; notes?: unknown }>(request, 4_096);
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Não foi possível atualizar o pedido." }, { status: requestErrorStatus(error, 400) });
  }
}
